...
```text
├── data/
│   ├── corpus.jsonl          # The raw industrial documentation
│   └── eval_queries.json     # The golden dataset for evaluation
├── ingest.py                 # Data cleaning and metadata extraction pipeline
├── retriever.py              # The core Hybrid RRF + Cross-Encoder search engine
├── evaluate.py               # Deterministic testing and metrics suite
├── requirements.txt          # Lightweight dependencies
└── README.md                 # Project documentation and decision log

1. Baseline Diagnosis
The provided baseline pipeline was fundamentally weak for industrial data due to three main issues:

Lack of Metadata Context: It embedded raw text without injecting the equipment ID. When a chunk just says "maximum pressure is 12 bar," the model loses the context of which machine it applies to.

Semantic-Only Vulnerability: Industrial docs rely on exact alphanumeric IDs (e.g., P-200, E-207). Pure dense vector search often struggles to isolate these exact matches, pulling semantically similar but incorrect machinery.

Zero Hallucination Defense: The baseline lacked an abstention mechanism. It would confidently return the closest vector match even if the user asked about a non-existent machine.

2. Data Quality Policy
Like real industrial documentation, the corpus.jsonl contained artifacts.

The Issue: System extraction tags like `` were polluting the raw text, which warps the mathematical placement of the document in the dense vector space.
...
Instead of using an expensive, LLM-as-a-judge framework, I built a deterministic evaluation suite (evaluate.py) against a custom golden dataset (data/eval_queries.json).
...
Tools Used: Google Gemini 1.5 Pro.

Purpose: Used as a pair-programming architect to brainstorm hybrid RRF mathematics, regex formatting for the data cleaner, and structural best practices for the offline evaluation script without relying on external libraries like Ragas.
also i use Ai to write README.md

Changes Made: I adjusted the weighting in the RRF function to better prioritize semantic hits over lexical hits for this specific dataset. I also enforced the switch from a heavy BGE-M3 model back to a MiniLM model to strictly adhere to the compute constraints.

Concrete Mistake Caught: During the drafting of the data ingestion script, the AI output a broken regex string (r'\') intended to clean the source tags, which resulted in a SyntaxError: unterminated string literal. I caught this, identified the escape-character conflict, and replaced it with the correct r'\' pattern to successfully clean the corpus.